...
$summaryJob = recent_job('send_periodic_summary');
...
<nav class="topbar">
<div class="brand"><div class="brand__mark">T</div><div><div class="brand__title"><?= h($title) ?></div><div class="brand__subtitle">Public NAV dashboard</div></div></div>
<div class="top-actions"><a class="nav-lock" href="protected/confirm/index.php">🔒 Confirm</a><div class="status-pill status-pill--<?= h($drawdownClass) ?>"><span class="status-dot"></span><?= $latest ? 'Data loaded' : 'No data yet' ?></div></div>
</nav>
<section class="hero-grid">
<div class="hero-copy"><div class="eyebrow">ProShares UltraPro QQQ</div><h1><?= h($ticker) ?> NAV Monitor</h1><p>Current NAV, all-time high and drawdown based on imported historical NAV data.</p></div>
<div class="hero-card hero-card--main"><div class="metric-label">Latest NAV</div><div class="metric-value"><?= $latestNav !== null ? format_decimal($latestNav, 4) : '–' ?></div><div class="metric-subline"><?= $latest ? h($latest['price_date']) : 'No import yet' ?><?php if ($dailyChange !== null): ?><span class="mini-change <?= $dailyChange >= 0 ? 'up' : 'down' ?>"><?= $dailyChange >= 0 ? '+' : '' ?><?= format_percent($dailyChange, 2) ?> vs. Prior NAV</span><?php endif; ?></div></div>
</section>
</header>
<main>
